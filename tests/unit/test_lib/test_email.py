...
@pytest.fixture
def email_service(mock_settings: MagicMock) -> Generator[EmailService, None, None]:
    """Create email service with mocked settings."""
    with (
        patch("sqlstack.lib.email.get_settings", return_value=mock_settings),
        patch("sqlstack.lib.settings.TEMPLATE_DIR", "/fake/templates"),
    ):
        yield EmailService()


@pytest.fixture
def test_user() -> s.User:
...
def test_initialization_with_settings(mock_settings: MagicMock) -> None:
...
def test_jinja_environment_setup(email_service: EmailService) -> None:
...
def test_smtp_connection_disabled(mock_settings: MagicMock) -> None:
...
@patch("smtplib.SMTP")
def test_smtp_connection_success(mock_smtp_class: MagicMock, email_service: EmailService) -> None:
...
def test_smtp_ssl_connection(mock_smtp_ssl_class: MagicMock, mock_settings: MagicMock) -> None:
...
@patch("smtplib.SMTP")
def test_smtp_connection_without_credentials(mock_smtp_class: MagicMock, mock_settings: MagicMock) -> None:
...
@patch("smtplib.SMTP")
def test_smtp_connection_failure(mock_smtp_class: MagicMock, email_service: EmailService) -> None:
...
async def test_send_email_disabled(mock_connection: MagicMock, mock_settings: MagicMock) -> None:
    ...
    with (
        ...
        result = await service.send_email(to_email="test@example.com", subject="Test", html_content="<p>Test</p>")
...
@patch.object(EmailService, "_create_smtp_connection")
async def test_send_email_no_connection(mock_connection: MagicMock, email_service: EmailService) -> None:
    """Test email sending when connection fails."""
    mock_connection.return_value = None

    result = await email_service.send_email(to_email="test@example.com", subject="Test", html_content="<p>Test</p>")

    assert result is False


@patch.object(EmailService, "_create_smtp_connection")
async def test_send_email_success(mock_connection: MagicMock, email_service: EmailService) -> None:
    """Test successful email sending."""
    mock_smtp = MagicMock()
    mock_connection.return_value = mock_smtp

    result = await email_service.send_email(
        to_email="test@example.com", subject="Test Subject", html_content="<p>Test HTML</p>", text_content="Test Text"
    )
...
@patch.object(EmailService, "_create_smtp_connection")
async def test_send_email_multiple_recipients(mock_connection: MagicMock, email_service: EmailService) -> None:
    """Test sending email to multiple recipients."""
    mock_smtp = MagicMock()
    mock_connection.return_value = mock_smtp

    recipients = ["test1@example.com", "test2@example.com"]
    result = await email_service.send_email(to_email=recipients, subject="Test", html_content="<p>Test</p>")
...
@patch.object(EmailService, "_create_smtp_connection")
async def test_send_email_auto_text_generation(mock_connection: MagicMock, email_service: EmailService) -> None:
    """Test automatic text content generation from HTML."""
    mock_smtp = MagicMock()
    mock_connection.return_value = mock_smtp

    result = await email_service.send_email(
        to_email="test@example.com", subject="Test", html_content="<h1>Title</h1><p>Paragraph with <b>bold</b> text</p>"
    )
...
@patch.object(EmailService, "_create_smtp_connection")
async def test_send_email_smtp_exception(mock_connection: MagicMock, email_service: EmailService) -> None:
...
@patch.object(EmailService, "_create_smtp_connection")
async def test_send_email_custom_sender(mock_connection: MagicMock, email_service: EmailService) -> None:
...
async def test_send_template_email_success(mock_send: MagicMock, email_service: EmailService) -> None:
    ...
    result = await email_service.send_template_email(
        template_name="welcome", to_email="test@example.com", subject="Welcome", context={"name": "John"}
    )

    assert result is True
    # Should call get_template twice: once for text template (fails), once for HTML template (succeeds)
...
@patch.object(EmailService, "send_email")
async def test_send_template_email_with_text_template(mock_send: MagicMock, email_service: EmailService) -> None:
...
@patch.object(EmailService, "send_email")
async def test_send_template_email_template_error(mock_send: MagicMock, email_service: EmailService) -> None:
    ...
    result = await email_service.send_template_email(
        template_name="nonexistent", to_email="test@example.com", subject="Test", context={}
    )
...
async def test_send_verification_email_with_template(
    mock_send_template: MagicMock, email_service: EmailService, test_user: s.User
) -> None:
...
@patch.object(EmailService, "send_template_email")
@patch.object(EmailService, "send_email")
async def test_send_verification_email_fallback(
    mock_send: MagicMock, mock_send_template: MagicMock, email_service: EmailService, test_user: s.User
) -> None:
...
async def test_send_password_reset_email(
    ...
    result = await email_service.send_password_reset_email(
        test_user, "reset-token-123", expires_in_minutes=60, ip_address="192.168.1.1"
    )
...
@patch.object(EmailService, "_create_smtp_connection")
async def test_send_email_with_empty_recipients(mock_connection: MagicMock, email_service: EmailService) -> None:
    """Test sending email with empty recipient list."""
    mock_smtp = MagicMock()
    mock_connection.return_value = mock_smtp

    result = await email_service.send_email(to_email=[], subject="Test", html_content="<p>Test</p>")

    # Should still try to send (SMTP will handle empty recipient list)
    assert result is True


async def test_user_without_name_in_verification_email(email_service: EmailService) -> None:
