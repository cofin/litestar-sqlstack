...
def test_app_settings_cors_origins_string_parsing() -> None:
    ...
    with patch.dict(os.environ, {"ALLOWED_CORS_ORIGINS": "http://localhost:3000,https://example.com"}):
        app_settings = AppSettings()
...
def test_app_settings_cors_origins_json_parsing() -> None:
    ...
    with patch.dict(os.environ, {"ALLOWED_CORS_ORIGINS": json.dumps(origins)}):
        app_settings = AppSettings()
        assert origins == app_settings.ALLOWED_CORS_ORIGINS
...
def test_settings_dotenv_loading(mock_load_dotenv: MagicMock, mock_is_file: MagicMock) -> None:
