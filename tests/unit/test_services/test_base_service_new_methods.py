...
def test_with_only_select_querybuilder_simple(driver: AsyncpgDriver) -> None:
...
def test_with_only_select_querybuilder_with_group_by(driver: AsyncpgDriver) -> None:
...
def test_with_only_select_querybuilder_with_where_and_having(driver: AsyncpgDriver) -> None:
...
def test_with_only_select_sql_object_with_expression(driver: AsyncpgDriver) -> None:
    """Test with_only_select with SQL object that has expression."""
    service = SQLSpecService(driver)

    # Create a real SQL expression string directly (testing the expression path)
    expression_sql = "SELECT id, name FROM users WHERE active = 1"

    # Transform to COUNT query
    count_sql = service.with_only_select(expression_sql)
...
def test_with_only_select_string_input(driver: AsyncpgDriver) -> None:
...
def test_with_only_select_non_select_statement(driver: AsyncpgDriver) -> None:
...
def test_with_only_select_invalid_sql(driver: AsyncpgDriver) -> None:
...
def test_with_only_select_parsing_error(mock_parse: MagicMock, driver: AsyncpgDriver) -> None:
...
def test_with_only_select_preserves_complex_where_conditions(driver: AsyncpgDriver) -> None:
...
def test_with_only_select_multiple_table_query(driver: AsyncpgDriver) -> None:
    ...
    # Should preserve FROM, WHERE clause and produce COUNT query
    # Note: SQLGlot may optimize the JOIN away when only counting, but should preserve the core query structure
    assert "COUNT(*)" in count_sql
    assert "users" in count_sql
    assert "active" in count_sql
    # ORDER BY should be removed
