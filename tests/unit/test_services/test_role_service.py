...
async def test_get_role_by_id(role_service: RoleService, test_role: s.Role) -> None:
...
async def test_get_role_not_found(role_service: RoleService) -> None:
    """Test retrieving a non-existent role raises error."""
    non_existent_id = uuid4()

    with pytest.raises(ValueError):  # Assuming service raises ValueError for not found
        await role_service.get_one(non_existent_id)


async def test_update_role(role_service: RoleService, test_role: s.Role) -> None:
...
async def test_delete_role(role_service: RoleService, test_role: s.Role) -> None:
...
async def test_list_roles(role_service: RoleService) -> None:
...
async def test_get_by_name(role_service: RoleService, test_role: s.Role) -> None:
...
async def test_get_by_name_not_found(role_service: RoleService) -> None:
...
async def test_get_by_slug(role_service: RoleService, test_role: s.Role) -> None:
...
async def test_exists_by_name(role_service: RoleService, test_role: s.Role) -> None:
    """Test checking if a role exists by name."""
    exists = await role_service.exists_by_name(test_role.name)
    assert exists is True

    exists_not = await role_service.exists_by_name("Non-existent Role")
    assert exists_not is False


async def test_get_default_role(role_service: RoleService) -> None:
    """Test getting the default user role."""
...
async def test_ensure_default_roles(role_service: RoleService) -> None:
    """Test ensuring default roles exist."""
    await role_service.ensure_default_roles()

    # Check that both default roles exist
