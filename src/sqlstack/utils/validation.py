...
This module provides validators for business logic that goes beyond structural
validation. Use these in combination with the semantic types from
``sqlstack.utils.types`` which handle basic structural constraints via msgspec.Meta.

Validation Layers:
    1. **Structural** (msgspec.Meta): Length, pattern - automatic at decode time
    2. **Business Logic** (this module): Blocked domains, common passwords, etc.

Example:
    >>> from sqlstack.utils.validation import validate_email
    >>> validate_email("user@example.com")
    'user@example.com'
    >>> validate_email("user@mailinator.com")
    ValidationError: Email domain not allowed
"""
