...
class DomainServiceProvider(Provider):
    ...
    @provide
    def get_user_oauth_account_service(self, driver: AsyncDriverAdapterBase) -> UserOAuthAccountService:
...
class WorkerPersistenceProvider(Provider):
    ...
    Note: The Worker class manages its own database connections directly
    for better performance. This provider is for other DI needs in worker context.
    """
...
def make_worker_container() -> AsyncContainer:
    ...
    """
    return make_async_container(WorkerPersistenceProvider(), DomainServiceProvider(), skip_validation=True)
