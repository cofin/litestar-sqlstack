...
from sqlstack.services import FilterTypes, OffsetPagination, TagService
...
class TagController(Controller):
    ...
    tags = ["Tags"]
    signature_types = [TagService, s]

    @get(operation_id="ListTags")
    @inject
    async def list_tags(
        self, tags_service: Inject[TagService], filters: Annotated[list[FilterTypes], Dependency(skip_validation=True)]
    ...
    async def get_tag(
        self,
        tags_service: Inject[TagService],
    ...
    @inject
    async def create_tag(self, tags_service: Inject[TagService], data: s.TagCreate) -> s.Tag:
        """Create a new tag.
